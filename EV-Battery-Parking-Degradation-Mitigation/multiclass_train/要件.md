...
1) **時間・曜日**  
   ...
   - `hour_sin, hour_cos`：`hour` を 24h円環に射影（sin/cos）。  
   **必要性**：人の行動は**日/週周期**に強く依存。帰宅帯（夜）・出勤帯（朝〜日中）の強い差を学習可能。  
   **計算**：`h = hour + minute/60`; `sin(2πh/24)`, `cos(2πh/24)`。

2) **充電メタ**  
   - `station_type`（200V/急速のOne-Hot）  
   - `charge_durations_minutes`（充電の滞在長）  
   - `soc_start, soc_end, soc_delta`（充電前後のSOCと増分）  
   **必要性**：急速/長時間充電の別、SOC余力は**この後の移動/放置の意図**の強い示唆。
...
1) **距離** `dist_to_Cj`  
   - **計算**：充電地点（`charge_lat, charge_lon`）と Cj重心（train内の中位/平均座標）の **ハバーサイン距離[km]**。  
     スケーリングは `log1p` を推奨（地理の歪み緩和）。  
...
4) **時間相性（簡易版）** `time_compat_Cj`（**中央値遅延 × 4hビン**）  
   ...
   - **スコア化**：  
     - `time_compat_Cj = p_start_Cj[到着(d,b)]`  
     - 幅許容：隣接ビンを0.5重みで加算してもよい（ズレ吸収）。  
